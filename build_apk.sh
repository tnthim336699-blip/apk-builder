...
if command -v docker >/dev/null 2>&1; then
  ...
  # 在容器中运行 buildozer，自动继续（传入 'y' 以接受许可证提示）
  docker run --rm -v "$BUILD_TMP":/work -w /work \
    --entrypoint /bin/bash kivy/buildozer:latest -c "
    buildozer -v android debug < /dev/null || (echo 'y' | buildozer -v android debug)
  "
